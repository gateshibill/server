layui.use(['form', 'layer', 'table', 'laytpl'], function () {
    ...
    var tableIns = table.render({
        elem: '#adminList',
        method: 'post',
        url: '/memberRiskControl/getwarninglist',
        cellMinWidth: 95,
        page: true,
       // height: "full-125",
        limits: [10, 15, 20, 25],
    ...
    function pingcang(data){
        $.post('/memberRiskControl/pc',{
            userId : data.userId
        },function(data){
            if(data.msg == 'success'){
...
//持仓
    ...
    table.on('tool(adminList)', function (obj) {
        ...
        }else if(layEvent == 'pingcang'){
            layer.confirm('确认同意此会员平仓？',{icon:3, title:'提示信息'},function(index){
                pingcang(data);
            });
        }
    });
